%profile on
% all times are in ms
lag = 20; % accounts for ability delay
...
%If dummy module is off, 5% Vulnerable debuff is applied by Mind Crush or Vanquish and does not ever fall off. Vulnerable is applied before their damage takes place, so the damage is affected by it. No need to implement it for this reason.
%Telekinetic Balance - tkt doesn't have a cd - OK
%Psychic Barrier - tkt returns 1% of the force - this would push down minimum force needed to activate tkt to 10, but for safety measure it's kept at 24. If you were extremely unlucky with accuracy you might not be able to get force back at a decent rate which might push force into negative numbers. Regardless, this should be an extremely border case scenario OK
forceSuppression = balance; % 10% to next 15 dots - OK
presenceOfMind = balance; %presence of mind proc - OK
%abundantJustice = balance; %overwhelmed buff, similar reasoning as for Vulnerable - OK
...
setbonus2 = 1; % 15 seconds of MA, 2 cost off tkt and Disturbance
...
mainStatBase = 240*12 + 550; % mainstat from armorings and mods; before class buff
powerBase = 12*90 + 2*41 + 2*52; %power from enhancements, crystals and relics
forcetechpowerBase = 1395*2;
accuracyBase = 0;
criticalBase = 0;
alacrityBase = 0;
surgBase = 0;
...
tic
%inc=100; % stat increase
%dpsinc=zeros(1000/inc+1,4);
istat=1; % outer loop iteration
iterations=1000; % inner loop iterations

for alacrityBase=200
    ...
    parfor j=1:iterations
        ...
        supp = 0; %suppression stacks, checks are only supp>0
        ...
        execute = 0; %for dot increase under 30% hp
        ...
        power = powerBase;
        forcetechpower = forcetechpowerBase;
        ...
        Power = (power + stim2 + forcetechpower);
        ...
        % bonusHealing = (Power*0.17 + mainStat*0.14) * (1 + bonusDmgBuff);
        % mainHandBonus = 29.6 + power*0.23;
        % mainHand = mainHandHilt + mainHandBonus;
        ...
        % OPENWE
        %init (check [acc alacbase crit surge bonusDmg] = stats(accuracy, alacrity, critical, surg, Power, mainStat, powerBuff, critBuff, forceGift, penetratingLight, innerStrengthAcc) ) -> mind crush -> sever force -> weaken mind -> fib -> tk throw -> check
        %priority  (no lag needed)
        if balance==1
        ...
        while t<420000 && i<2000
            ...
            action = keytimes(2,1);
            keytimes = [keytimes(:,2:end),[1000000; 1000000]]; %deletes first one, inserts placeholder on last spot
            
            switch action
                case -2 % natural force regen each second
                    force = min(forcePool, force + 8*(1+alac)*(1+mcont/10)); %min(600, x+regen)
                    T=t+1000;
                ...
                case -1 % decides about next ability to use
                    ...
                    if balance>0
                        ...
                        mc = (cd_mc<=t) * 20 * (force >= mccost*(1-0.75*(tkEff>0))) * (cd_tg2>t); %mind crush, last part is about the proc that makes it 0.5s faster
                        tb = (cd_tu<=t) * 18 * (force >= tucost*(1-0.75*(tkEff>0))); % turbulence
                ...
                case 5 % mind crush
                    ...
                    if damage>0
                        ...
                        if mentalContinuum==1 && cd_wm > t %weaken mind refresh
                            ...
                            wmalac = alac; %so WM ticks at the rate dicated by alacrity value upon refresh
                        end
                        tkEff = tkEff-1;
                ...
                case 9 % weaken mind application
                    if (-rand + (1-boss_defense) + acc > 0)>0
...
%                     end
                ...
                case 27 % turbulence
                    ...
                    if cd_fm<t && setbonus1==1 %force master 2% damage proc - also applies if it's a resist
                        ...
                        Tspot=find(T<keytimes(1,:),1);
                        keytimes = [keytimes(:,1:Tspot-1), [T; 31], keytimes(:,Tspot:end-1)];
                        cd_fm = t + 30000/(1+alac); %check for alacrity
                    end
                ...
                if relic2>0
                    if cd_mp<t % mainstat proc relic, no double proc bug
                        if rand < chance
                            ...
                            T=t;
                            Tspot=find(T<keytimes(1,:),1);
                            keytimes = [keytimes(:,1:Tspot-1), [T; 0], keytimes(:,Tspot:end-1)];                            cd_mp = t+20000/(1+alac);
                            T=t+6000;
                            Tspot=find(T<keytimes(1,:),1);
...
%                 end
            ...
            log(:,i) = [t; action; damage; force]; %basic combat log for debugging purposes
            i = i+1;
...
%          end
    ...
    meandps = mean(dps(:,1));
    maxdps = max(dps(:,1));
    dpsinc(istat,:) = [powerBase criticalBase, accuracyBase alacrityBase, surgBase, mindps meandps maxdps];
    
    istat=istat+1;
    disp([powerBase, criticalBase, accuracyBase alacrityBase, surgBase]);
end
toc
